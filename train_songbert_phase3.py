...
from src.Phase3Dataset import PlaylistDataset3, PlaylistData3Collator
...
def main():
    ...
    print("\nCreating datasets...")
    train_dataset = PlaylistDataset3(train_list, total_songs, tokenizer, songid_to_tokenized, args.num_context_songs, args.num_samples, args.name_length)
    val_dataset = PlaylistDataset3(val_list, total_songs, tokenizer, songid_to_tokenized, args.num_context_songs, args.num_samples, args.name_length)
    ...
    print("\nInitializing model...")
    model = SongBertModelPhase3(base_model, loss_fn=torch.nn.BCEWithLogitsLoss()).to(device)

    wandb.login(key="b8f04920d3894ec74f250663f7e4faef217c52a0") # Vrinda's key
    ...
    trainer = Trainer(
        ...
        data_collator=PlaylistData3Collator(),
